...
export function ControlPanel() {
  ...
  const [textareaHeight, setTextareaHeight] = useState<number>(UI_CONSTANTS.CONTROL_PANEL.MIN_TEXTAREA_HEIGHT)
  ...
  return (
    <>
      <div className="fixed bottom-0 left-0 lg:left-[80px] right-0 z-40 px-4 pb-[calc(1rem+env(safe-area-inset-bottom))] lg:pb-4 pointer-events-none flex flex-col items-center bg-background/80 backdrop-blur-md border-t border-border/60">
        <div className="w-full max-w-4xl relative pointer-events-auto flex flex-col items-center">
          {!isExpanded && (
            <div className="absolute -top-10 w-full flex justify-end z-50 pointer-events-none max-w-[600px] mx-auto left-0 right-0">
              <Button
                variant="secondary"
                className="rounded-full h-7 px-3 text-[11px] font-medium bg-card border border-border text-muted-foreground hover:text-foreground transition-colors pointer-events-auto"
                onClick={() => {
                  setIsExpanded(true)
          ...
          <div className={cn(
            "bg-card border border-border relative mx-auto transition-all ease-in-out rounded-2xl",
            isExpanded ? "w-full max-w-4xl p-4" : "w-full max-w-[600px] p-3"
          )}
          style={{
            ...
            <div className="relative group flex items-center w-full">
              <Textarea
                ...
                placeholder="描述你想生成的画面，按 Enter 发送，Shift+Enter 换行…"
                className={cn(
                  "border-none focus-visible:ring-0 resize-none bg-transparent text-sm placeholder:text-muted-foreground/60 w-full transition-all leading-relaxed",
                  isExpanded ? "py-3 px-1 pb-2" : "py-3 px-1 pr-12 overflow-hidden cursor-pointer"
                )}
                style={{ height: isExpanded ? textareaHeight + 'px' : '40px', minHeight: isExpanded ? '88px' : '40px', maxHeight: '280px' }}
                onClick={() => !isExpanded && setIsExpanded(true)}
                ref={textareaRef}
              />

              <div className={cn(
                "absolute right-0 transition-all duration-200 flex items-center justify-center",
                isExpanded ? "opacity-0 pointer-events-none scale-90" : "opacity-100 pointer-events-auto scale-100"
              )}>
                <Button
                  size="icon"
                  className={cn(
                    "h-9 w-9 rounded-full transition-colors z-10",
                    prompt.trim() ? "bg-primary text-primary-foreground hover:bg-primary/90" : "bg-secondary text-muted-foreground hover:bg-secondary/80",
                    isGenerating && "bg-secondary text-muted-foreground cursor-not-allowed"
                  )}
                  onClick={(e) => {
                    e.stopPropagation()
                    handleGenerate()
            ...
            >
              <div className="overflow-hidden min-h-0">
                ...
                <div className="flex items-center justify-between px-1 gap-2 overflow-x-auto no-scrollbar">
                  <div className="flex items-center gap-2 shrink-0">
                    <Select value={selectedModel} onValueChange={(val) => setModel(val || "")}>
                      <SelectTrigger className="h-9 min-w-[160px] max-w-[200px] bg-secondary border-0 rounded-xl text-xs font-medium hover:bg-secondary/80 transition-colors focus:ring-1 focus:ring-ring">
                    ...
                    <Popover>
                        ...
                        render={(props) => (
                          <Button
                            ...
                            className={cn(
                              ...
                              selectedStyles.length > 0 && "text-primary bg-primary/10 border-primary/20 hover:bg-primary/15"
                            )}
                          >
                      ...
                      />
                      <PopoverContent className="w-80 p-0 overflow-hidden rounded-xl border-border shadow-lg" align="start" side="top" sideOffset={8}>
                        ...
                        <div className="p-2 max-h-[300px] overflow-y-auto grid grid-cols-2 gap-1">
                          {availableStyles.map(style => (
                            <div
                              ...
                              className={cn(
                                "cursor-pointer px-3 py-2 rounded-xl text-xs transition-colors flex items-center justify-between group",
                    ...
                    <Popover>
                      <PopoverTrigger
                        render={(props) => (
                          <Button
                            ...
                            variant="ghost"
                            size="sm"
                            className="h-9 rounded-xl text-xs font-medium hover:bg-secondary text-muted-foreground hover:text-foreground"
                          >
                      ...
                      />
                      <PopoverContent className="w-80 p-4 rounded-xl border-border shadow-lg" align="end" side="top" sideOffset={8}>
                        <div className="space-y-5">

                          <div className="space-y-3">
                            <div className="flex items-center justify-between">
                              <Label className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider">画幅比例</Label>
                              <span className="text-[10px] font-mono text-muted-foreground">{width}x{height}</span>
                            </div>
                            <div className="grid grid-cols-5 gap-2">
                              {RATIO_LIST.map(r => (
                                <button
                                  ...
                                  className={cn(
                                    "flex flex-col items-center justify-center p-1 rounded-xl border transition-colors h-13",
                          ...
                          <div className="space-y-3">
                            <div className="flex items-center justify-between">
                              <Label className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider">生成数量</Label>
                          ...
                          <div className="space-y-3">
                            <div className="flex items-center justify-between">
                              <Label className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider">迭代步数</Label>
                          ...
                          <div className="space-y-3">
                            <div className="flex items-center justify-between">
                              <Label className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider">提示词相关性</Label>
                  ...
                  <Button
                    size="sm"
                    className={cn(
                      "h-9 px-4 rounded-xl font-medium transition-colors active:scale-95 bg-primary text-primary-foreground hover:bg-primary/90",
                      isGenerating && "bg-secondary text-muted-foreground cursor-not-allowed"
                    )}
