...
ExecStart=/bin/bash -c 'if [ ! -d ".next" ]; then npm run build >> $LOG_OUT 2>> $LOG_ERR; fi && mkdir -p $LOG_DIR && if [ -f .next/standalone/server.js ]; then node .next/standalone/server.js >> $LOG_OUT 2>> $LOG_ERR; elif [ -f .next/standalone/ui/server.js ]; then node .next/standalone/ui/server.js >> $LOG_OUT 2>> $LOG_ERR; else npm run start; fi >> $LOG_OUT 2>> $LOG_ERR'
Restart=on-failure
RestartSec=3
